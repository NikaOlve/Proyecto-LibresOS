...
function insertar_estudiante($ci, $nombres, $apellidos, $carrera, $id_facultad, $mail, $id_usuario) {
    $conexion = new Conexion();
    $statement = 'INSERT INTO estudiante (ci,nombres,apellidos, carrera, id_facultad, mail, id_usuario) VALUES (?,?,?,?,?,?,?)';
    $consulta = $conexion->prepare($statement);
    if ($consulta->execute(array($ci, $nombres, $apellidos, $carrera, $id_facultad, $mail, $id_usuario))) {
        return true;
    } else {
        return false;
    }
